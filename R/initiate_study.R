...
initiate_study <- function(
  ...
  if (
    !is.list(outcomes) || is.null(names(outcomes)) || any(names(outcomes) == "")
  ) {
    stop(
      "`outcomes` must be a fully named list (every element needs a non-empty name)."
    )
  }
  if (
    !is.null(extra_vars) &&
  ...
  extract_from_sel <- function(sel, nm_for_error) {
    if (is.character(sel)) {
      ...
      if (length(oob)) {
        stop(sprintf(
          "'%s': index/indices out of range: %s",
          nm_for_error,
          paste(oob, collapse = ", ")
        ))
      }
  ...
  cluster_data <- if (!is.null(cluster_idx)) {
    extract_block(cluster_idx)
  } else {
    NULL
  }
  ...
  treat_results <- if (is_df) {
    lapply(treat_col_names, function(nm) {
      classify_treatment(treatment_raw[[nm]], nm)
    })
  } else {
    lapply(seq_len(ncol(treatment_raw)), function(i) {
      classify_treatment(treatment_raw[, i], treat_col_names[[i]])
    })
  }
  ...
  trt_user <- resolve_labels(
    treatment_labels,
    treat_col_names,
    "treatment_labels"
  )
  resolved_treatment_labels <- ifelse(
    trt_user != treat_col_names,
    trt_user,
    trt_auto
  )
  ...
  resolved_time_labels <- if (!is.null(time_data)) {
    resolve_labels(time_labels, colnames(time_data), "time_labels")
  } else {
    NULL
  }
  resolved_cluster_labels <- if (!is.null(cluster_data)) {
    resolve_labels(cluster_labels, colnames(cluster_data), "cluster_labels")
  } else {
    NULL
  }
...
print.nana_task <- function(x, ...) {
  ...
  for (nm in names(x$outcomes)) {
    ...
    cens_str <- if (nm %in% names(x$censoring)) {
      sprintf("  [%d censored]", sum(x$censoring[[nm]] == 0L))
    } else {
      ""
    }
    cat(sprintf(
